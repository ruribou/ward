...
- `name` must match `^sys_[a-z]+(_[a-z]+)*$` — the `sys_` prefix, then lowercase words
  joined by single underscores (no digits).
...
Following the existing `sys_pull_image` example:
...
it back — see `sys_pull_image` / `sys_remove_image`, which are each other's inverse.
...
honest. They check that every op uses the `sys_` name convention, has a known risk class,
