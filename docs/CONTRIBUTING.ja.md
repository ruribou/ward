...
## 承認後に自動継続する（`ward wait`）

承認は **out-of-band** です——人間が自分のターミナルで `ward approve <id>` を叩くので、その完了はエージェントのループに戻ってきません。`ward wait <id>` は、その人間のステップを**自動化することなく**、エージェントが**そこでブロックして待つ**ための仕組みです。

想定するエージェントのループ：

1. エージェントが mutating 操作を提案し、提案 id `pN` を受け取る。
2. エージェントは `ward wait pN` を**バックグラウンド**プロセスとして起動し、そこでブロックする。
3. 人間が自分のターミナルで `ward approve pN`（または `ward reject pN`）を実行する。これが提案を消費し、`ward wait` は保留ストアを read-only でポーリングしてそれに気づく。
4. 提案がストアから消えると `ward wait` プロセスは `0` で終了する。エージェントは次の手を提案する前に、**read-only 操作で結果を確認する**。承認が _失敗_ しても提案は消費されるため、`0` 終了は「ホストの変更が成功した」ではなく「人間が処理した」を意味する——エージェントはホストが実際に変わったかを確認しなければならない。

`ward wait` は **read-only** です：提案ストアに対して `.get`／`.list` を呼び、id が保留集合から消えたことを観察するだけです。承認も、操作の実行も、ストアへの書き込みも一切しません。**承認はあくまで人間**です——次の mutating 操作にも人間の `ward approve` が要ります。タイムアウト（既定 600 秒）すると `ward wait` は `124` で終了し、何も変わっていません。

> Phase 2（監査ログの `exitCode` 精緻化——`ward wait` が承認の成功と失敗を区別できるようにする）は #50 に依存し、この変更には含まれません。
