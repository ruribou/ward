...
- `name` は `^nuc_[a-z]+$` に一致——`nuc_` プレフィックスの後は小文字英字のみ（数字や追加の `_` は不可）。
...
良い `plan` は、何が変わるか・既に済んでいる場合に no-op か・どう巻き戻すかを述べます——互いに逆操作である `nuc_pull` / `nuc_rmi` を参照。
...
`src/tests/registry/operations.test.ts` のレジストリテストが操作を honest に保ちます。すべての操作が `nuc_` 命名規約に従うこと、既知の risk クラスを持つこと、シェルメタ文字の無い定数コマンドを持つこと、そして——parity ガード——**すべての操作が各ロケールに `title` と `description` を持つこと**、**すべての mutating 操作が各ロケールに `plan` と空でない `precheck` を持つこと**を確認します。ローダ自身のガード（危険な引数・未知の risk・重複名・fail-closed ロード）もそこで網羅されます。日本語ラベルを忘れたり禁止文字を使ったりすると、これらのテストが正確に場所を教えてくれます。
